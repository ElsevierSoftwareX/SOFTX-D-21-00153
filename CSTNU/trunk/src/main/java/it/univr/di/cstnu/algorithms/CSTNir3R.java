...
public class CSTNir3R extends CSTNir {
	...
	public static void main(final String[] args) throws IOException, ParserConfigurationException, SAXException {
		...
		final CSTNir3R cstn = new CSTNir3R();
		...
		if (cstn.versionReq) {
			System.out.println(CSTNir3R.class.getName() + " " + VERSIONandDATE + ". Academic and non-commercial use only.\n"
	 ...
	 * This method differs from {@link CSTNir#initAndCheck()} because it does not apply R0 and R3*.
	 * 
	...
	public boolean initAndCheck() throws WellDefinitionException {
		...
		for (final LabeledIntEdge e : this.g.getEdges()) {
			...
			if (e.isEmpty()) {
				...
				if (Debug.ON) {
					if (LOG.isLoggable(Level.WARNING)) {
						LOG.log(Level.WARNING, "Label fixing on edge " + e + " removed all labels. Edge " + e + " has been removed.");
					}
				}
				continue;
			}
		...
		for (final LabeledIntEdge e : this.g.getInEdges(Z)) {
			...
			if (s.isObserver()) {
				this.labelModificationqR0(s, Z, e);
			}
			this.labelModificationqR3(s, Z, e);
			if (s.isObserver()) {
	...
	boolean labeledPropagationqLP(final LabeledNode nA, final LabeledNode nB, final LabeledNode nC, final LabeledIntEdge eAB,
			final LabeledIntEdge eBC, LabeledIntEdge eAC) {
		throw new IllegalAccessError("labeledPropagationqLP cannot be used in a CSTNir3R. Only labeledPropagationLP!");
	}
	...
	@Override
	boolean labelModificationR0qR0(final LabeledNode nObs, final LabeledNode nX, final LabeledNode nZ, final LabeledIntEdge ePZ) {
		throw new IllegalAccessError("R0 cannot be used in a CSTNir3R. Only qR0!");
	}

	@Override
	boolean labelModificationR3qR3(final LabeledNode nS, final LabeledNode nD, final LabeledNode nZ, final LabeledIntEdge eSZ) {
		throw new IllegalAccessError("R3* cannot be used in a CSTNir3R");
	}

	/**
	 * On 2017-10-16 this method has been simplified for working only with qR0. The previous code has been comment out!<br>
		...
		for (LabeledIntEdge AB : edgesToCheck) {
			...
			if (B == Z) {
				...
				if (A.isObserver()) {// R3 can add new values that have to be minimized. Experimentally VERIFIED on June, 28 2015
					// R0 on the resulting new values
					this.labelModificationqR0(A, Z, AB);
				}
			...
			for (LabeledIntEdge CA : this.g.getInEdges(A)) {
				...
				this.labeledPropagationLP(C, A, B, CA, AB, CB);
		...
		for (LabeledNode A : this.g.getVertices()) {
			...
			for (LabeledIntEdge AB : this.g.getOutEdges(A)) {
				...
				if (B == Z) {
					...
					if (A.isObserver()) {
						// R0 on the resulting new values
						this.labelModificationR0qR0(A, B, Z, AB);
					}
				...
				for (LabeledIntEdge BC : this.g.getOutEdges(B)) {
					...
					if (C == Z) {
						if (B.isObserver()) {
							// R0 on the resulting new values
							this.labelModificationR0qR0(B, C, Z, BC);
						}
					...
					if (C == Z) {
						if (A.isObserver()) {
							// R0 on the resulting new values
							this.labelModificationR0qR0(A, C, Z, AC);
						}
