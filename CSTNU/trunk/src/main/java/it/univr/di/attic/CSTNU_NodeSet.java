 ...
 * FIXME: it has to be reviewed after the stabilization of CSTN_NodeSet.
 * 
 * @author Roberto Posenato
 * @version $Id: $Id
 */
@Deprecated
public class CSTNU_NodeSet {
	/**
	 ...
	 * logger
	 */
	static Logger LOG = Logger.getLogger(CSTNU_NodeSet.class.getName());

	/**
	 * Shortcut
	 */
	static int nullInt = Constants.INT_NULL;
	...
	static boolean checkNodeLabelsSubsumption(final LabeledNode source, final LabeledNode dest, final Label newLabel, final String edgeName,
		...
		if ((source == null) || (dest == null) || (newLabel == null) || (edgeName == null)) {
			CSTNU_NodeSet.LOG.warning("One parameter is null. source: " + source + ", dest: " + dest + ", new label to add: " + newLabel + ", edgeName: "
		...
		if (!newLabel.subsumes(labelConjunction)) {
			CSTNU_NodeSet.LOG.warning("Subsumption check for a label generated by rule " + ruleName + " on edge " + edgeName + ".\nThe new label, '"
	...
	static boolean checkWellDefinition1Property(final LabeledNode tail, final LabeledNode head, final LabeledIntEdge e) throws WellDefinitionException {
		if ((e == null) || (tail == null) || (head == null)) {
			CSTNU_NodeSet.LOG.warning("One parameter is null at least. Please, check parameter.");
			return false;
		}
		...
		for (final Object2IntMap.Entry<Label> entry : e.getLabeledValueMap().entrySet()) {
			if (!entry.getKey().subsumes(labelConjunction)) {
				CSTNU_NodeSet.LOG.warning("Labeled value " + entry + " of edge " + e.getName() + " does not subsume the endpoint labels.");
		...
		for (final Object2IntMap.Entry<Entry<Label, String>> entry : e.getUpperLabelSet()) {
			if (!entry.getKey().getKey().subsumes(labelConjunction)) {
				CSTNU_NodeSet.LOG.warning("Upper case Labeled value " + entry + " of edge " + e.getName() + " does not subsume the endpoint labels.");
				throw new WellDefinitionException(
		...
		for (final Object2IntMap.Entry<Entry<Label, String>> entry : e.getLowerLabelSet()) {
			if (!entry.getKey().getKey().subsumes(labelConjunction)) {
				CSTNU_NodeSet.LOG.warning("Lower case Labeled value " + entry + " of edge " + e.getName() + " does not subsume the endpoint labels.");
				throw new WellDefinitionException(
	...
	static boolean checkWellDefinition2Property(final LabeledIntGraph g, final LabeledNode node) throws WellDefinitionException {
		...
		for (final char l : nodeLabel.getPropositions()) {
			...
			if (obs == null) {
				CSTNU_NodeSet.LOG.warning("Observation node of literal " + l + " of node " + node + " does not exist.");
			...
			if (!nodeLabel.subsumes(obsLabel)) {
				CSTNU_NodeSet.LOG.warning("Label of node " + node + " does not subsume label of obs node " + obs);
			...
			if ((e == null) || (e.getMinValue() == CSTNU_NodeSet.nullInt) || (e.getMinValue() > 0)) {
				CSTNU_NodeSet.LOG.warning("There is no constraint to execute obs node " + obs + " before node " + node);
	...
	static boolean checkWellDefinition3Property(final LabeledIntGraph g, final LabeledIntEdge e) throws WellDefinitionException {
		...
		for (final Object2IntMap.Entry<Entry<Label, String>> entry : allLabeledValuesSet) {
			...
			for (final char l : edgeLabel.getPropositions()) {
				...
				if (obs == null) {
					CSTNU_NodeSet.LOG.warning("Observation node of literal " + l + " present in label " + edgeLabel + " of edge " + e
							+ " does not exist.");
				...
				if (!edgeLabel.subsumes(obsLabel)) {
					CSTNU_NodeSet.LOG.warning("Label " + edgeLabel + " of edge " + e + " does not subsume label of obs node " + obs);
	...
	static boolean checkWellDefinitionProperties(final LabeledIntGraph g) throws WellDefinitionException {
		boolean flag = false;
		CSTNU_NodeSet.LOG.info("Checking if graph is well defined...");
		for (final LabeledIntEdge e : g.getEdges()) {
			flag = CSTNU_NodeSet.checkWellDefinition1Property(g.getSource(e), g.getDest(e), e);
			flag = flag && CSTNU_NodeSet.checkWellDefinition3Property(g, e);
		}
		for (final LabeledNode node : g.getNodes()) {
			flag = flag && CSTNU_NodeSet.checkWellDefinition2Property(g, node);
		}
		CSTNU_NodeSet.LOG.info(((flag) ? "done: all is well defined.\n" : "done: something is wrong. Not well defined graph!\n"));
		return flag;
	}
	 ...
	 */
	private static Label getGammaCleaned(final Label gamma, final char p, final LabeledIntGraph g) {
		if ((gamma == null) || (g == null))
			return null;
		final Label gamma1 = new Label();
		for (final Literal l : gamma.getLiterals()) {
			final LabeledNode obs = g.getObservator(l.getName());
			CSTNU_NodeSet.LOG.finest("Observation for " + l + " in gamma: " + obs);
			final Label obsLabel = obs.getLabel();
			CSTNU_NodeSet.LOG.finest("Label for " + obs.getName() + ": " + obsLabel);

			if (obsLabel.getStateLiteralWithSameName(p) == State.absent) {
				gamma1.conjunct(l);
			}
			CSTNU_NodeSet.LOG.finest("Gamma1 update to:" + gamma1);
		}
		return gamma1;
	}

	/**
	 * Returns the sum of all negative values (ignoring their labels) present in the edges of a graph. If an edge has more than one negative values, only the
	 * minimum among them is considered. For contingent link, also the lower case value is considered.
	 *
	 * @param g
	...
	private static int getSumOfNegativeEdgeValues(final LabeledIntGraph g) {
		...
		for (final LabeledIntEdge e : g.getEdges()) {
			...
			if ((min != CSTNU_NodeSet.nullInt) && (min < 0)) {
				// if (minLC != null) {
		...
		CSTNU_NodeSet.LOG.finer("The sum of all negative values is " + sum);
		return sum;
	}
	...
	LabeledIntGraph makeAllMaxProjection(final LabeledIntGraph g) {
		if (g == null)
			return null;
		final LabeledIntGraph distance = new LabeledIntGraph(g, labeledIntValueMap);
	 ...
	 * If yes (in all edges!), then the rule adds to each Z&lt;---O edge, the labeled value (l', min), where min is the less negative value among the checked
	 * labeled values and l' is the label formed making the conjunction of all the checked label removing all opposite literals and literal associated to the
	 * considered observation nodes.<br>
	 * If no, returns.
	 *
	...
	static boolean negativeQStarR6(final LabeledIntGraph currentGraph, final LabeledIntGraph nextGraph) {
		CSTNU_NodeSet.LOG.finer("Label Modification R6: start.");
		if ((currentGraph == null) || (nextGraph == null)) {
			CSTNU_NodeSet.LOG.info("One parameter is null. Game over");
			return false;
		}
		final Char2ObjectArrayMap<LabeledNode> obsMap = currentGraph.getObservedAndObservator();
		if (obsMap == null)
			return false;
		...
		for (final LabeledNode obsNode : obsNodes) {
			...
			final char l = obsNode.getPropositionObserved();
			...
			for (final it.unimi.dsi.fastutil.objects.Object2IntMap.Entry<Label> entryObsZ : eObsZ.labeledValueSet()) {
				...
				final CharArraySet literalRemoved = new CharArraySet();
				literalRemoved.add(l);
				final Label newLabel = new Label();
				final ObjectArraySet<LabeledIntEdge> edgesToUpdate = new ObjectArraySet<>();

				for (final Literal l1 : label.getLiterals()) {
					final LabeledNode othObs = currentGraph.getObservator(l1.getName());
					if (othObs == null)
					...
					for (final it.unimi.dsi.fastutil.objects.Object2IntMap.Entry<Label> entryOthObsZ : eOthObsZ.labeledValueSet()) {
						...
						if (label1.contains(l)) {
							...
							found = true;
							for (final Literal ll : label1.getLiterals()) {
								if (l2.isConsistentWith(ll)) {
									l2.conjunct(ll);
								} else {
									l2.remove(ll);
									literalRemoved.add(ll.getName());
								}
							}
						}
					}
					if (found) {
						literalRemoved.add(l1.getName());
						newLabel.remove(l1);
						newLabel.remove(l1.getComplement());
						for (final char ll : literalRemoved) {
							l2.remove(ll);
						}
						for (final Literal ll : l2.getLiterals()) {
				...
				for (final LabeledIntEdge e : edgesToUpdate) {
					CSTNU_NodeSet.LOG.finer("R6 is adding to " + e + " the following value: (" + newLabel + ", " + lessNegative + ")...");
					if (e.mergeLabeledValue(newLabel, lessNegative)) {
						CSTNU_NodeSet.LOG.finer("done!");
					} else {
						CSTNU_NodeSet.LOG.finer("NOT DONE because already done in the past");
					}
				}
				// CSTNU.R6calls++;
			}
		}
		CSTNU_NodeSet.LOG.finer("Label Modification R6: end.");
		return false;
	}

	/**
	 * OLD VERSION; IT DOES NOT MANAGE ALL POSSIBLE CASES Applies the Negative QStar Rules, also known as R6.<br>
	 * In this method all values are negative! Determine the less negative value (max value) among the set of negative value built considering, for each edge
	 * ObservationNode-->Z, the max negative value regardless the label (min value).
	 *
	 * @param currentGraph the originating graph.
	 * @param nextGraph the graph where to write the new constraints.
	 * @return true if one rule has been applied one time at least.
	 ...
	 */
	static private LabeledIntEdgePluggable newEdgeInCSTNU(String name, LabeledIntEdge.ConstraintType type, LabeledIntGraph g) {
		int i = g.getEdgeCount();
		String name1 = new String(name);
		while (g.getEdge(name1) != null) {
			name1 = name + "_" + i++;
		}
		LabeledIntEdgePluggable e = g.getEdgeFactory().create();
		e.setName(name1);
	...
	static private int sumAndOverflowCheck(final int a, final int b, final Label l) {
		if ((a == Constants.INT_NULL) || (b == Constants.INT_NULL) || (l == null))
	...
	boolean caseLabelRemovalRule(final LabeledIntGraph currentGraph, final LabeledIntGraph nextGraph, CSTNUCheckStatus status) {
		...
		CSTNU_NodeSet.LOG.finer("Label Removal Rule: start.");
				...
				for (final Object2IntMap.Entry<Entry<Label, String>> upperCaseEntryOfCA : CA.getUpperLabelSet()) {
					...
					for (final Object2IntMap.Entry<Entry<Label, String>> lowerCaseEntryOfAB : ABMap) {
						...
						if (CAInNext.mergeLabeledValue(l1l2, z)) {
							...
							CSTNU_NodeSet.LOG.finer("Case Label Removal applied to edge " + oldCA + ":\n" + "partic: " + B.getName()
									+ " <---(" + l1 + ", "
							...
							if (z != oldZ) {
								CSTNU_NodeSet.LOG.finer("LabeledIntEdge " + CA.getName() + " after the rule: " + CA.toString());
							}
						}
					}
				}
		...
		CSTNU_NodeSet.LOG.finer("Label Removal Rule: end.");
	...
	boolean crossCaseRule(final LabeledIntGraph currentGraph, final LabeledIntGraph nextGraph, CSTNUCheckStatus status) {
		...
		CSTNU_NodeSet.LOG.finer("Cross Case Rule: start.");
		for (final LabeledIntEdge DC : lowerCaseEdges) {
			...
			for (final LabeledIntEdge e : currentGraph.getOutEdges(C)) {
				if (((v = e.getMinUpperLabeledValue()) != CSTNU_NodeSet.nullInt) && (v <= 0)) {// FIXME: if instantaneousReaction, v < 0 ?
				...
				for (final Object2IntMap.Entry<Entry<Label, String>> entryLabelUpperCaseEdge : upperCaseEdge.getUpperLabelSet()) {
					...
					for (final Object2IntMap.Entry<Entry<Label, String>> entryLowerCaseEdgeToC : DCMap) {
						...
						if (l1l2 == null)
							continue;
						if (DA == null) {
							DA = newEdgeInCSTNU(D.getName() + "_" + A.getName(), LabeledIntEdge.ConstraintType.derived, nextGraph);
							nextGraph.addEdge(DA, nextGraph.getNode(D.getName()), nextGraph.getNode(A.getName()));
							// nextGraph.addUpperLabeledEdge(DA);
							CSTNU_NodeSet.LOG.finer("crossCaseRule: added edge " + DA.getName());

						}
						final int z = CSTNU_NodeSet.sumAndOverflowCheck(x, y, l1l2);
						...
						if (DA.mergeUpperLabelValue(l1l2, upperCaseNodeName, z)) {
							...
							CSTNU_NodeSet.LOG.finer("Cross Case applied to edge " + oldDA + ":\n partic: " + A.getName() + " <---(" + l1
									...
									+ oldZ);
							if (z != oldZ)
								CSTNU_NodeSet.LOG.finer("LabeledIntEdge " + DA.getName() + " after the ruel: " + DA.toString());
						}
					}
				}
			}
		}
		CSTNU_NodeSet.LOG.finer("Cross Case Rule: end.");
		return reductionApplied;
	}
	...
	public CSTNUCheckStatus dynamicControllabilityCheck(final LabeledIntGraph g) throws WellDefinitionException {
		...
		distanceGraph = new LabeledIntGraph(labeledIntValueMap);
		...
		CSTNU_NodeSet.LOG.info("The maximum number of possible cycles is " + maxCycles);

		int i;

		long startTime = System.nanoTime();
		for (i = 1; i <= maxCycles && status.controllable && !status.finished; i++) {
			if (CSTNU_NodeSet.LOG.isLoggable(Level.FINE))
				CSTNU_NodeSet.LOG.log(Level.FINE, "*** Start Main Cycle " + i + "/" + maxCycles + " ***");
			...
			if (CSTNU_NodeSet.LOG.isLoggable(Level.FINE))
				CSTNU_NodeSet.LOG.log(Level.FINE, "*** End Main Cycle " + i + "/" + maxCycles + " ***\n\n");
		}
		status.executionTime = (System.nanoTime() - startTime) / 1000000;

		if (!status.controllable) {
			if (CSTNU_NodeSet.LOG.isLoggable(Level.INFO)) {
				CSTNU_NodeSet.LOG.log(Level.INFO, "After " + (i - 1) + " cycle, found an INCONTROLLABILITY.\nStatus: " + status);
				CSTNU_NodeSet.LOG.log(Level.FINER, "Final un-controllable graph: " + nextGraph);
			}
			g.takeIn(nextGraph);
		...
		if (i > maxCycles && !status.finished) {
			if (CSTNU_NodeSet.LOG.isLoggable(Level.INFO)) {
				CSTNU_NodeSet.LOG.log(Level.INFO, "The maximum number of cycle (+" + maxCycles + ") has been reached!\nStatus: " + status);
				CSTNU_NodeSet.LOG.log(Level.FINER, "Last determined graph: " + nextGraph);
			}
		...
		if (CSTNU_NodeSet.LOG.isLoggable(Level.INFO)) {
			CSTNU_NodeSet.LOG.log(Level.INFO, "Stable state reached. Number of cycles: " + (i - 1) + " over the maximum allowed " + maxCycles
					+ ".\nStatus: " + status);
		}
	...
	/**
	 * Initializes all data structures before the execution of controllability checking algorithm. This method has to be called before the application of any
	 * propagation/reduction rule. If the given instance does not contain a node named 'Z' (assumed to be the first node to be executed), the method adds it and
	 * all the necessary temporal for assuring that Z will be the first executed node.
	 * 
	 * @param g a cstnu instance to check.
	 * @throws IllegalArgumentException if the graph is null or it is not well formed.
	 */
	@SuppressWarnings("static-method")
	public void initUpperLowerLabelDataStructure(final LabeledIntGraph g) throws IllegalArgumentException {
		...
		for (final LabeledIntEdge e : edgeSet) {
			...
			if (CSTNU_NodeSet.LOG.isLoggable(Level.FINEST))
				CSTNU_NodeSet.LOG.log(Level.FINEST, "Edge e: " + e);
			...
			if (CSTNU_NodeSet.LOG.isLoggable(Level.FINEST)) {
				CSTNU_NodeSet.LOG.log(Level.FINEST, "Source label: " + s.getLabel() + "; dest label: " + d.getLabel() + " new label: " + conjunctLabel);
			}
			if (conjunctLabel == null) {
				CSTNU_NodeSet.LOG.warning("Found a inconsistent label between the endopoint of edge " + e + ". Edge removed!");
				g.removeEdge(e);
				continue;
			}
			if (!conjunctLabel.isEmpty()) {
				...
				for (Object2IntMap.Entry<Label> entry : e.labeledValueSet()) {
					...
					if (l1.conjunction(conjunctLabel) == null) {
						CSTNU_NodeSet.LOG.warning("Found a labeled value in " + e + " inconsistent with the conjunction of node labels, "
								+ conjunctLabel + ". Labeled value removed");
						e.removeLabel(l1);
					} else {
						if (!l1.subsumes(conjunctLabel)) {
							CSTNU_NodeSet.LOG.warning("Found a labeled value in " + e
			...
			try {
				CSTNU_NodeSet.checkWellDefinition3Property(g, e);
			 ...
			 */
			/**
			 * From svn version 74 I try to introduce the management of guarded link (contingent link with different paired requirement constraints). The idea
			 * is to set the upper/lower case label if and only if there is NO upper/lower case label.
			 */
			final int initialValue = e.getMinValueConsistentWith(conjunctLabel);
			if (initialValue == CSTNU_NodeSet.nullInt)
			...
			CSTNU_NodeSet.LOG.finer("Edge " + e + " is contingent. Found its companion: " + eInverted);
			if (eInverted == null) {
				// Since
			...
			if ((initialValue <= 0) && (eInverted.getLowerLabelValue(conjunctLabel, s) == CSTNU_NodeSet.nullInt)) {
				CSTNU_NodeSet.LOG.warning("Edge " + e + " is contingent with a negative value but the inverted " + eInverted
						+ " does not contain a lower case value: (" + conjunctLabel + ", " + s.getName() + ").");
				insertUpperLowerCaseValue = true;
			}
			if ((initialValue > 0) && (eInverted.getUpperLabelValue(conjunctLabel, d) == CSTNU_NodeSet.nullInt)) {
				CSTNU_NodeSet.LOG.warning("Edge " + e + " is contingent with a positive value but the inverted " + eInverted
			...
			if (insertUpperLowerCaseValue) {
				if (initialValue <= 0) {
					...
					CSTNU_NodeSet.LOG.warning("Insert the lower label value: (" + conjunctLabel + ", " + s.getName().toLowerCase() + ", "
							+ (-initialValue) + ") to edge " + eInverted.getName());
				} else {
					eInverted.mergeUpperLabelValue(conjunctLabel, d, -initialValue);
					CSTNU_NodeSet.LOG.warning("Insert the upper label value: (" + conjunctLabel + ", " + s.getName().toUpperCase() + ", "
							+ (-initialValue) + ") to edge " + eInverted.getName());
		...
		if (Z == null) {
			Z = g.getNode(CSTNU_NodeSet.ZeroNodeName);
			if (Z == null) {
				// We add by authority!
				Z = new LabeledNode(CSTNU_NodeSet.ZeroNodeName);
				Z.setX(0.0);
				Z.setY(0.0);
				g.addVertex(Z);
				if (CSTNU_NodeSet.LOG.isLoggable(Level.WARNING))
					CSTNU_NodeSet.LOG.log(Level.WARNING, "No " + CSTNU_NodeSet.ZeroNodeName + " node found: added!");
			}
			g.setZ(Z);
		}
		...
		for (final LabeledNode node : nodeSet) {
			...
			if (obs != Constants.UNKNOWN) {
				if (label.contains(obs)) {
					if (CSTNU_NodeSet.LOG.isLoggable(Level.WARNING))
						CSTNU_NodeSet.LOG.log(Level.WARNING, "Literal '" + obs + "' cannot be part of the label '" + label
			...
			try {
				CSTNU_NodeSet.checkWellDefinition2Property(g, node);
			} catch (final WellDefinitionException ex) {
				if (ex.getType() == Type.ObservationNodeDoesNotOccurBefore) {
					for (final char l1 : label.getPropositions()) {
						...
						if (e == null) {
							e = newEdgeInCSTNU(node.getName() + "_" + obsl1.getName(), LabeledIntEdge.ConstraintType.derived, g);
							g.addEdge(e, node, obsl1);
							CSTNU_NodeSet.LOG.warning("It is necessary to add a preceding constraint between node '" + node.getName()
		...
		if (!Z.getLabel().isEmpty()) {
			if (CSTNU_NodeSet.LOG.isLoggable(Level.WARNING))
				CSTNU_NodeSet.LOG.log(Level.WARNING, "In the graph, Z node has not empty label. Label removed!");
		...
		for (final LabeledNode node : nodeSet) {
			...
			if (e == null) {
				e = newEdgeInCSTNU(node.getName() + "_" + Z.getName(), LabeledIntEdge.ConstraintType.derived, g);
				g.addEdge(e, node, Z);
				CSTNU_NodeSet.LOG.info("It is necessary to add a preceding constraint between node '" + node.getName() + "' and node '" + Z.getName()
	 ...
	 *
	 * @param currentGraph
	 * @param status CSTNUCheckStatus representing the status of the checking algorithm.
	...
	boolean labelModificationR0R2(final LabeledIntGraph currentGraph, final CSTNUCheckStatus status) {
		...
		CSTNU_NodeSet.LOG.finer("Label Modification R0 and R2: start.");
		for (final LabeledNode obsNode : obsNodes) {
			...
			for (final LabeledIntEdge edge : outEdges) {
				...
				for (final Object2IntMap.Entry<Label> entryObs : edgeLabeledValueSet) {
					...
					if (edge.getValue(l) == CSTNU_NodeSet.nullInt) {
					...
					if (w == 0 && !CSTNU_NodeSet.checkNodeLabelsSubsumption(obsNode, destNode, labelWithouP, edge.getName(), "R0")) {
					...
					if (edge.mergeLabeledValue(labelWithouP, w)) {
						CSTNU_NodeSet.LOG.finer(logMessage);
					}
				}
				...
				for (final Object2IntMap.Entry<Entry<Label, String>> entryObs : edgeUpperLabeledValueSet) {
					...
					if (edge.getUpperLabelValue(l, nodeName) == CSTNU_NodeSet.nullInt) {
				...
				if (r0Applied) {
					CSTNU_NodeSet.LOG.finer("LabeledIntEdge " + edge.getName() + " after the rule R0: " + edge.toString());
					ruleApplied = true;
				}
			}
			...
			for (final LabeledIntEdge edgeInModification : inEdges) {
				...
				for (final Object2IntMap.Entry<Label> entryObs : edgeLabeledValueSet) {
					...
					if (edgeInModification.getValue(l) == CSTNU_NodeSet.nullInt) {
						continue;// it is possible that in a previous cycle the
									// label has been
					}
					...
					if (w > 0) {
						// R2 rule
						if (!CSTNU_NodeSet.checkNodeLabelsSubsumption(sourceNode, obsNode, labelWithouP, edgeInModification.getName(), "R2")) {
							...
							CSTNU_NodeSet.LOG.finer("Details about subsumption check of R2 applied to edge " + edgeInModification
						...
						// R2 special CASE
						if (!CSTNU_NodeSet.checkNodeLabelsSubsumption(sourceNode, obsNode, labelWithouP, edgeInModification.getName(), "R2")) {
							CSTNU_NodeSet.LOG.finer("R2 CANNOT be applied because node label to " + edgeInModification + ": "
				...
				for (final Object2IntMap.Entry<Entry<Label, String>> entryObs : edgeUpperLabeledValueSet) {
					...
					final Label l = entryObs.getKey().getKey();
					final String nodeName = entryObs.getKey().getValue();
					if (edgeInModification.getUpperLabelValue(l, nodeName) == CSTNU_NodeSet.nullInt) {
					...
					if (w > 0) {
						...
						if (!CSTNU_NodeSet.checkNodeLabelsSubsumption(sourceNode, obsNode, labelWithouP, edgeInModification.getName(), "R2")) {
							...
							// The labeled value has to be substituted by a 0 labeled value because by WD2 the node ha to be
							// after the observation node.
							CSTNU_NodeSet.LOG.finer("Details about subsumption check of R2 to " + edgeInModification + ":\npartic: "
						...
						// R2 special case
						if (!CSTNU_NodeSet.checkNodeLabelsSubsumption(sourceNode, obsNode, labelWithouP, edgeInModification.getName(), "R2")) {
							CSTNU_NodeSet.LOG.finer("Details because R2 cannot be applied to " + edgeInModification + obsNode.getName()
				...
				if (r2Applied) {
					CSTNU_NodeSet.LOG.finer("LabeledIntEdge " + edgeInModification.getName() + " after the rule R2: "
							+ edgeInModification.toString());
					ruleApplied = true;
				}
			}
		}
		CSTNU_NodeSet.LOG.finer("Label Modification R0 and R2: end.");
		return ruleApplied;
	}
	 ...
	 * @param eObsX LabeledIntEdgePluggable connecting observation node and x.
	 * @param p the Observation proposition (it is redundant... only to speed up the method)
	 * @param eXYnew LabeledIntEdgePluggable that will contain the new determined labels.
	...
	private boolean labelModificationR1Action(final LabeledNode obs, final LabeledNode nX, final LabeledNode nY, final LabeledIntEdge eXY,
			final LabeledIntEdge eObsX, final char p, final LabeledIntEdge eXYnew, final LabeledIntGraph g, CSTNUCheckStatus status) {
		if (eXY == null)
			return false;
		...
		CSTNU_NodeSet.LOG.finest("obs_xLabeledValueSet: " + obs_xLabeledValueSet.toString());
		...
		for (final Object2IntMap.Entry<Label> entryObs : obs_xLabeledValueSet) {
			...
			if ((w > 0) || (l.getStateLiteralWithSameName(p) != State.absent)) {
			...
			for (final Object2IntMap.Entry<Entry<Label, String>> entryXY : x_yLabeledValueSet) {
				...
				if (l1.getStateLiteralWithSameName(p) == State.absent) {
					continue;
				}
				...
				l1WithoutP.remove(p);
				final Label[] alphaBetaGammaPart = CSTNU_NodeSet.getAlphaBetaGamma(l, l1WithoutP, nX != g.getZ());
				final Label alpha = alphaBetaGammaPart[0];
				final Label beta = alphaBetaGammaPart[1];
				final Label gamma1 = CSTNU_NodeSet.getGammaCleaned(alphaBetaGammaPart[2], p, g);
				...
				if (abg1 == null) {
					final String msg = "The label alpha beta gamma is null and it cannot be null: \n" + "alpha: " + alpha + "; beta: "
							+ beta + "; gamma': " + gamma1;
					CSTNU_NodeSet.LOG.severe(msg);
					throw new IllegalArgumentException(msg);
				}
				if (!CSTNU_NodeSet.checkNodeLabelsSubsumption(nX, nY, abg1, eXY.getName(), "R1")) {
					...
					CSTNU_NodeSet.LOG.finer("Detail about the error of application R1 to edge " + eXY + ":\npartic: " + obs.getName() + " ---(" + l
				...
				if (r1Applied) {
					CSTNU_NodeSet.LOG.finer("R1 adds a label to edge " + eXY + ":\npartic: " + obs.getName() + " ---(" + l + ", "
							+ Constants.EMPTY_UPPER_CASE_LABELstring + ", " + w + ")---> " + nX.getName() + " ---(" + l1 + ", "
							+ UCLabelXY + ", " + v + ")---> " + nY.getName() + "\nresult: " + nX.getName() + " ---(" + abg1 + ", "
							+ UCLabelXY + ", " + v + ")---> " + nY.getName());
					ruleApplied = true;
		...
		if (ruleApplied) {
			CSTNU_NodeSet.LOG.finer("LabeledIntEdge " + eXYnew.getName() + " after R1 application: " + eXYnew.toString());
		}
	 ...
	 * {@link #labelModificationR1Action(LabeledNode, LabeledNode, LabeledNode, LabeledIntEdgePluggable, LabeledIntEdgePluggable, Literal, LabeledIntEdgePluggable, LabeledIntGraph, CSTNUCheckStatus)}
	 * and
	 * {@link #labelModificationR3Action(LabeledNode, LabeledNode, LabeledNode, LabeledIntEdgePluggable, LabeledIntEdgePluggable, Literal, LabeledIntEdgePluggable, LabeledIntGraph, CSTNUCheckStatus)}
	 *
	 * @param currentGraph the originating graph.
	 * @param nextGraph the graph where to write the new constraints.
	 * @param status CSTNUCheckStatus object representing the status of a checking algorithm run.
	 * @return true if one rule has been applied one time at least.
	 * @see CSTNU_NodeSet#labelModificationR1Action(LabeledNode, LabeledNode, LabeledNode, LabeledIntEdgePluggable, LabeledIntEdgePluggable, Literal,
	 *      LabeledIntEdgePluggable, LabeledIntGraph, CSTNUCheckStatus)
	 * @see CSTNU_NodeSet#labelModificationR3Action(LabeledNode, LabeledNode, LabeledNode, LabeledIntEdgePluggable, LabeledIntEdgePluggable, Literal,
	 *      LabeledIntEdgePluggable, LabeledIntGraph, CSTNUCheckStatus)
	 */
	@SuppressWarnings("javadoc")
	boolean labelModificationR1R3(final LabeledIntGraph currentGraph, final LabeledIntGraph nextGraph, CSTNUCheckStatus status) {
		...
		CSTNU_NodeSet.LOG.finer("Label Modification R1 and R3: start.");
		for (final LabeledNode obsNode : obsNodes) {
			final char p = obsNode.getPropositionObserved();
			...
			for (final LabeledIntEdge eObsX : currentGraph.getOutEdges(obsNode)) { // node
				...
				if ((eObsX.getMinValue() == CSTNU_NodeSet.nullInt) || (eObsX.getMinValue() > 0)) {
		...
		CSTNU_NodeSet.LOG.finer("Label Modification R1R3: end.");
		return ruleApplied;
	}
	 ...
	 * @param nX x node
	 * @param nY y node
	 * @param eYX LabeledIntEdgePluggable containing the constrain to modify
	 * @param eObsX LabeledIntEdgePluggable connecting observation node and x.
	 * @param p the Observation proposition (it is redundant... only to speed up the method)
	 * @param eYXnew LabeledIntEdgePluggable that will contain the new determined labels.
	...
	private boolean labelModificationR3Action(final LabeledNode nObs, final LabeledNode nX, final LabeledNode nY, final LabeledIntEdge eYX,
			final LabeledIntEdge eObsX, final char p, final LabeledIntEdge eYXnew, final LabeledIntGraph g, CSTNUCheckStatus status) {
		if (eYX == null)
			return false;
		...
		for (final Object2IntMap.Entry<Entry<Label, String>> entryObsX : obs_xLabeledValueSet) {
			...
			for (final Object2IntMap.Entry<Entry<Label, String>> entryYX : y_xLabeledValueSet) {
				...
				final Label beta = alphaBetaGammaPart[1];
				final Label gamma1 = CSTNU_NodeSet.getGammaCleaned(alphaBetaGammaPart[2], p, g);
				CSTNU_NodeSet.LOG.finest("Rule R3 details alpha=" + alpha);
				CSTNU_NodeSet.LOG.finest("Rule R3 details beta=" + beta);
				CSTNU_NodeSet.LOG.finest("Rule R3 details gamma1=" + gamma1);
				final Label abg1 = alpha.conjunction(beta).conjunction(gamma1);
				...
				// I check if the label subsumes the label of the endpoints before to proceed
				if (!CSTNU_NodeSet.checkNodeLabelsSubsumption(nY, nX, abg1, eYX.getName(), "R3")) {
					CSTNU_NodeSet.LOG.finer("Details because R3 cannot be applied to edge " + eYX + ":\n" + nObs.getName() + " ---(" + l + ", "
				...
				if (v >= w) {
					...
					if (r3Applied) {
						// R3 changed the edge
						CSTNU_NodeSet.LOG.finer("R3 adds a label to edge " + eYX + ":\n" + "partic: " + nObs.getName() + " ---(" + l + ", "
								+ UCLabelObsX + ", " + w + ")---> " + nX.getName() + " <---(" + l1 + ", " + UCLabelYX + ", " + v
		...
		if (ruleApplied) {
			CSTNU_NodeSet.LOG.finer("LabeledIntEdge " + eYXnew.getName() + " after R3 application: " + eYXnew.toString());
		}
		CSTNU_NodeSet.LOG.finest("Label Modification R3: end.");
		return ruleApplied;
	}
	...
	boolean lowerCaseRule(final LabeledIntGraph currentGraph, final LabeledIntGraph nextGraph, CSTNUCheckStatus status) {
		...
		boolean reductionApplied = false;
		Set<Object2IntMap.Entry<Entry<Label, String>>> DCMap;
		CSTNU_NodeSet.LOG.finer("Lower Case Rule: start.");
		for (final LabeledIntEdge DC : lowerCaseEdge) {
			...
			for (final LabeledIntEdge CA : currentGraph.getOutEdges(C)) {
				...
				if ((CA.getMinValue() == CSTNU_NodeSet.nullInt) || (CA.getMinValue() > 0))
					continue; // rule condition!
				...
				for (final Object2IntMap.Entry<Entry<Label, String>> entryDC : DCMap) {
					...
					for (final Object2IntMap.Entry<Label> entryCA : CAMap) {
						...
						if (DA == null) {
							DA = newEdgeInCSTNU(D.getName() + "_" + A.getName(), LabeledIntEdge.ConstraintType.derived, nextGraph);
							nextGraph.addEdge(DA, nextGraph.getNode(D.getName()), nextGraph.getNode(A.getName()));
							CSTNU_NodeSet.LOG.finer("lowerCaseRule: added edge " + DA.getName());
						}
						final int oldZ = DA.getValue(l1l2);
						final int z = CSTNU_NodeSet.sumAndOverflowCheck(y, x, l1l2);
						final String oldDA = DA.toString();

						if (DA.mergeLabeledValue(l1l2, z)) {
							reductionApplied = true;
							status.lowerCaseRuleCalls++;
							CSTNU_NodeSet.LOG.finer("Lower Case applied to edge " + oldDA + ":\n" + "partic: " + A.getName() + " <---(" + l1
									...
									+ "; old value: " + oldZ);
							if (z != oldZ) {
								CSTNU_NodeSet.LOG.finer("LabeledIntEdge " + DA.getName() + " after the rule: " + DA.toString());
							}
						}
					}
				}
		...
		CSTNU_NodeSet.LOG.finer("Lower Case Rule: end.");
		return reductionApplied;
	}

	/**
	 * Determines the minimal distance between all pairs of vertexes of the given graph if the graph does not contain any negative cycles. If the graph contains
	 * a negative cycle, the method stops and returns false (the graph is anyway modified).
	 *
	 * @param g the graph
	...
	public boolean minimalDistanceGraphFast(final LabeledIntGraph g) {
		...
		int v;
		Label l;
		LabeledIntNodeSetMap ijMap = null;
		for (int k = 0; k < n; k++) {
			kV = node[k];
			for (int i = 0; i < n; i++) {
				iV = node[i];
				for (int j = 0; j < n; j++) {
					...
					if ((k == i) || (k == j)) {
						ijMap = new LabeledIntNodeSetTreeMap(ij.getLabeledValueMap());// this is necessary to avoid concurrent access to the same map by the
																						// iterator.
					} else {
						ijMap = null;
					}
					for (final Object2IntMap.Entry<Label> ikL : ikMap) {
						for (final Object2IntMap.Entry<Label> kjL : kjMap) {
							...
							if (ij == null) {
								ij = newEdgeInCSTNU(node[i].getName() + "_" + node[j].getName(), LabeledIntEdge.ConstraintType.derived, g);
								g.addEdge(ij, iV, jV);
							}
							...
							if (i == j) // check negative cycles
								if (v < 0 || ij.getMinValue() < 0) {
									CSTNU_NodeSet.LOG.finer("Found a negative cycle on node " + iV.getName() + ": "
											+ ((ijMap != null) ? ijMap : ij) + "\nIn details, ik=" + ik + ", kj="
	...
	boolean noCaseRule(final LabeledIntGraph currentGraph, final LabeledIntGraph nextGraph, CSTNUCheckStatus status) {
		...
		CSTNU_NodeSet.LOG.finer("No-Case Rule: start.");
		for (int k = 0; k < n; k++) {
			C = node[k];
					...
					for (final Object2IntMap.Entry<Label> CAEntry : CAMap) {
						...
						for (final Object2IntMap.Entry<Label> DCEntry : DCMap) {
							...
							if (DA == null) {
								...
								CSTNU_NodeSet.LOG.finer("No-CaseRule: added edge " + DA.getName());
							}
							final int oldZ = DA.getValue(l1l2);
							final int z = CSTNU_NodeSet.sumAndOverflowCheck(x, y, l1l2);
							final String oldDA = DA.toString();
							if (DA.mergeLabeledValue(l1l2, z)) {
								ruleApplied = true;
								status.stdRuleCalls++;
								CSTNU_NodeSet.LOG.finer("No-Case applied to edge " + oldDA + ":\n" + "partic: " + A.getName() + " <---("
										+ l1 + ", ◇, " + x + ")--- " + C.getName() + " <---(" + l2 + ", ◇, " + y
										+ ")--- " + D.getName() + "\nresult: " + A.getName() + " <---(" + l1l2 + ", ◇, "
										+ z + ")--- " + D.getName() + "; old value: " + oldZ);
								if (z != oldZ) {
									CSTNU_NodeSet.LOG.finer("LabeledIntEdge " + DA.getName() + " after No-Case Rule: "
											+ DA.toString());
								}
							}
						}
		...
		CSTNU_NodeSet.LOG.finer("No-Case Rule: end.");
		return ruleApplied;
	}
	 ...
	 * @param nextGraph the next graph. It has be instantiate as a copy of current graph. It will contain the results of reduction at the end of the execution.
	 * @param distanceGraph the AllMax Projection of nextGraph. It will be instantiate by the method.
	 * @param status object representing the status of the check.
	...
	public CSTNUCheckStatus oneStepDynamicControllability(final LabeledIntGraph currentGraph, final LabeledIntGraph nextGraph,
		...
		LabeledIntGraph originalGraph = new LabeledIntGraph(labeledIntValueMap);
		originalGraph.copy(currentGraph);
		distanceGraph.copy(makeAllMaxProjection(currentGraph));
		CSTNU_NodeSet.LOG.finest("distanceGraph:" + distanceGraph);
		CSTNU_NodeSet.LOG.info("Current graph: " + currentGraph.getName());
		CSTNU_NodeSet.LOG.info("AllMax Projection check on " + distanceGraph.getName() + "...");
		if (!minimalDistanceGraphFast(distanceGraph)) {
			CSTNU_NodeSet.LOG.info("The AllMax Projection graph has at leat one negative loop at the start of cycle " + status.cycles + ": stop!");
			status.controllable = false;
			status.finished = true;
			return status;
		}
		CSTNU_NodeSet.LOG.info("AllMax Projection check done.\n");

		CSTNU_NodeSet.LOG.info("Label modifications phase...");
		...
		CSTNU_NodeSet.LOG.info("Label modifications phase done.\n");
		currentGraph.copy(nextGraph);
		currentGraph.setName("Current Graph");

		CSTNU_NodeSet.LOG.info("Rules phase...");
		...
		CSTNU_NodeSet.LOG.info("Rules phase done.\n");

		CSTNU_NodeSet.checkWellDefinitionProperties(nextGraph);

		if (!reductionApplied) {
			CSTNU_NodeSet.LOG.info("No more reductions applied at cycle " + status.cycles + ".\n");
			status.finished = true;
		}
		return status;
	}
	...
	boolean upperCaseRule(final LabeledIntGraph currentGraph, final LabeledIntGraph nextGraph, CSTNUCheckStatus status) {
		...
		CSTNU_NodeSet.LOG.finer("Upper Case Rule: start.");
				...
				for (final Object2IntMap.Entry<Entry<Label, String>> entryCA : CAMap) {
					...
					for (final Object2IntMap.Entry<Label> entryDC : DCMap) {
						...
						if (DA == null) {
							DA = newEdgeInCSTNU(D.getName() + "_" + A.getName(), LabeledIntEdge.ConstraintType.derived, nextGraph);
							nextGraph.addEdge(DA, nextGraph.getNode(D.getName()), nextGraph.getNode(A.getName()));
							CSTNU_NodeSet.LOG.finer("upperCaseRule: added edge " + DA.getName());
						}
						...
						if (DA.mergeUpperLabelValue(l1l2, upperCaseNode, z)) {
							...
							CSTNU_NodeSet.LOG.finer("Upper Case applied to edge " + oldDA + ":\n" + "partic: " + A.getName() + " <---(" + l1
		...
		CSTNU_NodeSet.LOG.finer("Upper Case Rule: end.");
		return reductionApplied;
	}
}
