 ...
 * In this class, the dynamic consistency check is done assuming standard DC semantics (cf. ICAPS 2016 paper, table 1).
...
public class CSTN {
	...
	Label makeBetaGammaDagger4qR3(final LabeledNode nS, final LabeledNode nZ, final LabeledNode nObs, final char observed,
		...
		char[] childrenOfP = this.g.getChildrenOf(nObs);
		if (childrenOfP != null) {
			for (char c : childrenOfP) {
				if (labelFromObs.contains(c)) {
					if (LOG.isLoggable(Level.FINEST)) {
						LOG.log(Level.FINEST,
								slog.toString() + " (β*γ)† cannot be calculated because labelFromObs contains a child of " + observed
