...
public class CSTN {
	 ...
	 * The {@link #dynamicConsistencyCheck(LabeledIntGraph, boolean, boolean)} calls this method before to execute the check.
	 *
	 * @param g a {@link it.univr.di.cstnu.graph.LabeledIntGraph} object.
	 * @param instantaneousReaction true if the CSTN g reacts instantaneously to observation of propositions.
	...
	public static boolean initAndCheck(final LabeledIntGraph g, boolean instantaneousReaction) {
			...
			try {
				...
				if (ex.getType() == Type.ObservationNodeDoesNotOccurBefore) {
					for (final Literal l1 : label.getAllAsStraight()) {
						...
						if (e == null) {
							e = new LabeledIntEdge("e" + node.getName() + obsl1.getName(), LabeledIntEdge.Type.derived, g.isOptimize());
		...
		for (final LabeledNode node : nodeSet) {
			...
			if (e == null) {
				e = new LabeledIntEdge("e" + node.getName() + Z.getName(), LabeledIntEdge.Type.derived, g.isOptimize());
				g.addEdge(e, node, Z);
	 ...
	 * @param excludeR1R2 true if one wants to exclude the application of rules R1 and R2. NOT YET APPLIED
	 *            the same 'instant'.
	...
	public static CheckStatus oneStepDynamicConsistency(final LabeledIntGraph currentGraph, final boolean instantaneousReaction, final boolean excludeR1R2,
		...
		/*
		 * Label Propagation
		 * Apply label propagation rule to all possible node triple (A,B,C).
		 * Exploiting a possible sparse graph, the triple are generated making, for each node B, two cycles:
		 * one to find all possible predecessors of B, each called A, and one to find all successors of B, each called C.
		...
		for (int k = 0; k < n; k++) {
			...
			for (final Iterator<LabeledIntEdge> eABIter = currentGraph.getInEdges(B).iterator(); eABIter.hasNext();) {
				...
				for (final Iterator<LabeledIntEdge> eBCIter = currentGraph.getOutEdges(B).iterator(); eBCIter.hasNext();) {
					...
					if (C == B) continue;// self loop on the second pair in not useful. The only loop that has to be maintain is A == C
					...
					if (!excludeR1R2) {
						...
						if (C.isObsevator()) CSTN.labelModificationR2(currentGraph, C, A, AC, status, instantaneousReaction);//It should be like R0! To verify experimentally.
					}
				}
			}
		}
	...
	static boolean checkWellDefinition1Property(final LabeledNode tail, final LabeledNode head, final LabeledIntEdge e) throws WellDefinitionException {
		...
		if (labelConjunction == null) {
			...
			if (CSTN.LOG.isLoggable(Level.WARNING)) CSTN.LOG.log(Level.WARNING, msg);
	...
	static boolean checkWellDefinition2Property(final LabeledIntGraph g, final LabeledNode node) throws WellDefinitionException {
		if ((g == null) || (node == null)) {
			if (CSTN.LOG.isLoggable(Level.WARNING)) CSTN.LOG.log(Level.WARNING, "One parameter is null at least. Please, check parameter.");
			return false;
		}
	...
	static boolean checkWellDefinition3Property(final LabeledIntGraph g, final LabeledIntEdge e) throws WellDefinitionException {
		...
		for (final Object2IntMap.Entry<Entry<Label, String>> entry : allLabeledValuesSet) {
			...
			if (edgeLabel.isEmpty()) continue;

			// check the observation node
			for (final Literal l : edgeLabel.getAllAsStraight()) {
				final LabeledNode obs = g.getObservator(l);
				if (obs == null) {
					final String msg = "Observation node of literal " + l + " present in label " + edgeLabel + " of edge " + e + " does not exist.";
					if (CSTN.LOG.isLoggable(Level.WARNING)) CSTN.LOG.log(Level.WARNING, msg);
					throw new WellDefinitionException(msg, WellDefinitionException.Type.ObservationNodeDoesNotExist);
				}
	...
	static boolean isNewLabeledValueNotANegativeLoopEspression(final Label newLabel, final int value, final LabeledIntEdge newEdge) {
		if ((value == Constants.INT_NEG_INFINITE) && !newLabel.containsUnknown()) {
			if (CSTN.LOG.isLoggable(Level.FINER)) CSTN.LOG.log(Level.FINER, "Found a negative loop that has determined a -∞ in the edge " + newEdge);
			return false;
		}
		return true;
	}
	...
	static boolean labelModificationR0(final LabeledIntGraph currentGraph, final LabeledNode P, final LabeledNode X, final LabeledIntEdge PX,
		...
		final ObjectArraySet<Label> obsXLabelSet = new ObjectArraySet<>(PX.getLabeledValueMap().keys());// It is necessary to have a copy of current labels
																										// for updating them without problems.
		for (final Label l : obsXLabelSet) {
			final int w = PX.getValue(l);
			if (w == LabeledIntNodeSetMap.INT_NULL) continue;// it is possible that in a previous cycle the label 'l' has been removed.
	...
	static boolean labelModificationR1(final LabeledIntGraph currentGraph, final LabeledNode nX, final LabeledNode nY, final LabeledIntEdge eXY,
		...
		if (edgeFromObsSet == null) return false;

		final ObjectArraySet<Label> XYLabelSet = new ObjectArraySet<>(eXY.getLabeledValueMap().keys());// It is necessary to have a copy of current labels for
		...
		for (LabeledIntEdge eObsX : edgeFromObsSet) {
			...
			for (Object2IntMap.Entry<Label> entryObsX : eObsX.labeledValueSet()) {
				...
				if (w > 0) continue; // R1 work with negative w.

				final Label obsXLabel = entryObsX.getKey();
				final Literal p = nObs.getPropositionObserved();

				for (final Label XYLabel : XYLabelSet) {
					if (!XYLabel.contains(p)) continue;
					final int v = eXY.getValue(XYLabel);
					if (v == LabeledIntNodeSetMap.INT_NULL) continue;// it is possible that in a previous cycle the label 'labelXY' has been removed.
					...
					if ((nY != Z) && (!obsXLabel.isConsistentWith(XYLabel) || obsXLabel.containsUnknown())) continue; // ... and when it is consistent with
					...
					if (!CSTN.checkNodeLabelsSubsumption(nX, nY, abg1, eXY.getName(), "R1")) {
						...
						if (CSTN.LOG.isLoggable(Level.FINER)) CSTN.LOG.log(Level.FINER, "Detail about the error of application R1 to edge " + eXY
								+ ":\nsource: "
								+ nObs.getName() + " ---(" + obsXLabel + ", " + Constants.formatInt(w) + ")---> " + nX.getName() + " ---(" + XYLabel + ", "
					...
					if (!(status.consistency = CSTN.isNewLabeledValueNotANegativeLoopEspression(abg1, v, eXY))) return ruleApplied;
				}
			}
		}
	...
	/**
	 * In presence of a label containing a proposition that can be decided only in the future, Rule 2 adds a new label one
	 * without the preposition and with a suitable value.
	 * In more details,
	 * 
	 * <pre>
	 * R2:
	...
	static boolean labelModificationR2(final LabeledIntGraph currentGraph, final LabeledNode P, final LabeledNode X, final LabeledIntEdge XP,
		...
		final ObjectArraySet<Label> XPLabelSet = new ObjectArraySet<>(XP.getLabeledValueMap().keys());// It is necessary to have a copy of current labels
		// for updating them without problems.
		for (final Label l : XPLabelSet) {
			...
			if (!CSTN.checkNodeLabelsSubsumption(X, P, alphaPrime, XP.getName(), "R2")) {
				if (w > 0) {
					...
					// The labeled value has to be substituted by a 0 labeled value because by WD2 the node has to be after the observation node.
					if (CSTN.LOG.isLoggable(Level.FINER))
			...
			status.r2calls++;
			ruleApplied = true;
			if (XP.mergeLabeledValue(alphaPrime, max) && CSTN.LOG.isLoggable(Level.FINER)) CSTN.LOG.log(Level.FINER, logMessage);
			if (!(status.consistency = CSTN.isNewLabeledValueNotANegativeLoopEspression(alphaPrime, max, XP))) return ruleApplied;
		}
	...
	static boolean labelModificationR3(final LabeledIntGraph currentGraph, final LabeledNode nX, final LabeledNode nY,
		...
		final ObjectArraySet<Label> YXLabelSet = new ObjectArraySet<>(eYX.getLabeledValueMap().keys());// It is necessary to have a copy of current labels for
																										// updating them without problems.
		for (LabeledIntEdge eObsX : edgeFromObsSet) {
			LabeledNode nObs = currentGraph.getSource(eObsX);
			...
			for (Object2IntMap.Entry<Label> entryObsX : eObsX.labeledValueSet()) {
				final int w = entryObsX.getIntValue();
				if (w > 0) continue; // R3 work with negative w.

				final Label labelObsX = entryObsX.getKey();
				final Literal p = nObs.getPropositionObserved();

				for (final Label YXLabel : YXLabelSet) {
					if (!YXLabel.contains(p)) continue;

					final int v = eYX.getValue(YXLabel);
					if (v == LabeledIntNodeSetMap.INT_NULL) continue;// it is possible that in a previous cycle the label 'labelYX' has been removed.

					if ((nX != Z) && !labelObsX.isConsistentWith(YXLabel)) continue;
	...
	static boolean labelPropagationRule(final LabeledIntGraph currentGraph, final LabeledNode A, final LabeledNode B, final LabeledNode C,
		...
		if (AC == null) {
			AC = new LabeledIntEdge("e" + A.getName() + C.getName(), LabeledIntEdge.Type.derived, currentGraph.isOptimize());
...
//									+ Constants.formatInt(y) + ")--> " + C.getName()
					...
					if (sum != Constants.INT_NEG_INFINITE) {
						...
						if (CSTN.LOG.isLoggable(Level.FINEST)) {
							...
							CSTN.LOG.log(Level.FINEST, "The new CANDIDATE node set to add to label '" + newLabelAC + "' on edge " + AC.getName()
									+ " is " + sigma);
						}
					}
				}
		...
		if (A == C) { // self loop, check if there is a negative loop!
			...
			if ((min != LabeledIntNodeSetMap.INT_NULL) && (min < 0)) {
				if (CSTN.LOG.isLoggable(Level.FINER)) CSTN.LOG.log(Level.FINER, "Found a negative loop in node " + AC.toString());
				status.finished = true;
				status.consistency = false;
			}
		}
		if (AC.labeledValueSet().isEmpty()) currentGraph.removeEdge(AC);
		return ruleApplied;
	}
	 ...
	 * opposite literals
	 * are represented as 'unknown' literals.
	 *
	 * @param currentGraph the current graph
	 * @param observator observator node
	 * @param observed the proposition observed by observator (since this value usually is already determined before calling this method, this parameter is just for speeding up.
	 ...
	 */
	static Label makeAlphaBetaGammaPrime(final LabeledIntGraph currentGraph, final LabeledNode observator, final Literal observed, final Label labelFromObs, final Label labelToClean,
			final boolean strict) {

		Label sourceLabel = new Label(labelToClean);
		sourceLabel.removeAllLiteralsWithSameName(observed);
		/*
		 * Label[0] is the sub-label of labelFromObs not in common with sourceLabel.<br>
		 * Label[1] is the sub-label of labelFromObs in common with sourceLabel.<br>
		 * Label[2] is the sub-label of labelFromObs not in common with sourceLabel. <br>
		 */
		...
		if (CSTN.LOG.isLoggable(Level.FINEST))
			CSTN.LOG.log(Level.FINEST, "labelEdgeFromObs = " + labelFromObs + ", labelWithoutP=" + sourceLabel + ",  alpha = " + alpha + ", beta=" + beta
					+ " gamma=" + gamma1);

		gamma1.removeAllLiteralsWithSameName(currentGraph.getChildrenOf(observator));
		if (CSTN.LOG.isLoggable(Level.FINEST)) CSTN.LOG.log(Level.FINEST, "gamma1=" + gamma1);
		if (strict) {
	...
	private static ObjectArraySet<LabeledIntEdge> getEdgeFromObservators(final LabeledIntGraph currentGraph, final LabeledNode nX) {
		int numObs;
		if (currentGraph == null || nX == null || (numObs = currentGraph.getObservators().size()) == 0) return null;
		...
		if (currentGraph.getIncidentEdges(nX).size() < numObs) {
			for (LabeledIntEdge e : currentGraph.getIncidentEdges(nX)) {
				if (currentGraph.getSource(e).isObsevator()) fromObs.add(e);
			}
		} else {
			LabeledIntEdge e;
		...
		if (fromObs.isEmpty()) return null;
		return fromObs;
	}
